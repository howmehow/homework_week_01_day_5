...
#############################################
  ...
  def test_sell_pet_to_customer__pet_found
    customer = @customers[0]
    pet = find_pet_by_name(@pet_shop,"Arthur")# we check if there is pet this name
    #if there is, then we can move the pet to customer

    sell_pet_to_customer(@pet_shop, pet, customer)

    # assert_equal(1, customer_pet_count(customer))
    assert_equal(1, pets_sold(@pet_shop)) #we update the pet_shop sold
    assert_equal(100, customer_cash(customer))#we reduce customer money
    assert_equal(1900, total_cash(@pet_shop))#and we add total cash
  end
  #
